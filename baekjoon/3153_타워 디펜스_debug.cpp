            ...
            if (m[i][j] == 'T') {
                ...
                for (int k=0;k<4;++k) {
                    if (find_t(i+dr[k], j+dc[k], k)) {
                        ...
                        printf("%d v %d (%d%c) at %d %d\n", oppo(s+k), oppo(s+k), t[i][j], "WENS"[k], i, j);
                    }
                }
            }
            else if (m[i][j] == 'n') {
                ...
                printf("%d v %d (%d%c, %d%c) at %d %d\n", t1+k1, t2+k2, ((t1+4)/4), "WENS"[k1], ((t2+4)/4), "WENS"[k2], i, j);
            }
        }
    }
